#!/bin/bash
# Figure 4-style PathMNIST classifier experiment:
# train ResNet-50 from scratch on Original, Naive SD 1/2/5/10x,
...
echo "============================================================"
echo "  PathMNIST ResNet-50 scratch scale training 시작"
...
run_original_seed() {
    local seed=$1
    local ckpt="${RESULT_ROOT}/original_only/seed${seed}"
    local log="logs/resnet_scale_original_seed${seed}.log"
...
run_expanded_seed() {
    ...
    local exp=$2
    local seed=$3
    local ckpt="${RESULT_ROOT}/${method}_lr${LR}/seed${seed}"
    local log_method="${method// /_}"
    local log="logs/resnet_scale_${log_method}_seed${seed}.log"
...
for ratio in ${RATIOS}; do
    for seed in ${SEEDS}; do
        run_expanded_seed "sd_batch_${ratio}x" "save/sd_batch_${ratio}x" "${seed}"
    done
done

for ratio in ${RATIOS}; do
    for seed in ${SEEDS}; do
        run_expanded_seed "distdiff_batch_${ratio}x" "save/distdiff_batch_${ratio}x" "${seed}"
    done
done

echo "============================================================"
echo "  PathMNIST ResNet-50 scratch scale training 완료"
echo "  $(date '+%Y-%m-%d %H:%M:%S')"
echo "============================================================"

echo
echo "=== Final results ==="
python src/parse_logs.py "${RESULT_ROOT}/original_only" --multi
for ratio in ${RATIOS}; do
    python src/parse_logs.py "${RESULT_ROOT}/sd_batch_${ratio}x_lr${LR}" --multi
done
for ratio in ${RATIOS}; do
    python src/parse_logs.py "${RESULT_ROOT}/distdiff_batch_${ratio}x_lr${LR}" --multi
done
