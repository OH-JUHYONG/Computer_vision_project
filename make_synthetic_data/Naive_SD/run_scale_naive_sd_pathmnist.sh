#!/bin/bash
# Generate PathMNIST naive Stable Diffusion synthetic data for 1/2/5/10x.
...
echo "============================================================"
echo "  PathMNIST Naive SD synthetic generation 시작"
...
for ratio in ${RATIOS}; do
    out_dir="data/${DATASET}_expansion/save/sd_batch_${ratio}x"
    ...
    if [ "${current}" -eq "${expected}" ]; then
        echo "[SKIP] Naive SD ${ratio}x already complete: ${out_dir}"
        continue
    fi
    if [ "${current}" -gt "${expected}" ]; then
        echo "[ERROR] Naive SD ${ratio}x has too many PNGs: ${current} / ${expected}" >&2
        exit 1
    fi

    for split in $(seq 0 $((TOTAL_SPLIT - 1))); do
        log_file="logs/naive_sd_generate_${ratio}x_split${split}.log"
        echo "[RUN] Naive SD ${ratio}x split ${split}/${TOTAL_SPLIT} -> ${log_file}"
    ...
    if [ "${final_count}" -ne "${expected}" ]; then
        echo "[ERROR] Naive SD ${ratio}x count mismatch. Check split logs." >&2
        exit 1
    fi
done
...
echo
echo "============================================================"
echo "  PathMNIST Naive SD synthetic generation 완료"
echo "  $(date '+%Y-%m-%d %H:%M:%S')"
echo "============================================================"
