...
> Project note: this working copy has been reorganized for the PathMNIST course project.
> Shared implementation files live under `src/`, synthetic-data launchers under
> `make_synthetic_data/`, and classifier launchers under `classifier/`.
> The upstream-style commands below are kept for reference; use the project
> structure summary in `PROJECT_STRUCTURE.md` for the current PathMNIST runs.
...
sh make_synthetic_data/DistDiff/expand_diff.sh 5 0 0 &
sh make_synthetic_data/DistDiff/expand_diff.sh 5 1 1 &
sh make_synthetic_data/DistDiff/expand_diff.sh 5 2 2 &
sh make_synthetic_data/DistDiff/expand_diff.sh 5 3 3 
```
* Please note that if your system doesn't possess 4 GPUs, you can modify the `total_split` parameter in `make_synthetic_data/DistDiff/expand_diff.sh` to reflect the actual number of GPUs you have, and make corresponding adjustments to the `SPLIT` and `GPU`.

**3) Training and Evaluation**
* After data expansion, use the following script to train and evaluate the backbone on the expanded datasets.

```
sh classifier/ResNet50_scratch/train_expand.sh ${DATASET} ${BACKBONE} 0.1 ${EXP} False ${GPU}
```
...
```
python src/parse_logs.py /path/to/exp --multi
```
