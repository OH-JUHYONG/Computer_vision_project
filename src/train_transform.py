...
parser.add_argument('--data_expanded_dir', default=None, type=str, metavar='PATH',
                    help='path to save checkpoint (default: checkpoint)')
parser.add_argument('--repeat_original', default=False, action='store_true',
                    help='repeat original data expand_num + 1 times when no expanded data is provided')
...
def train(train_loader, model, criterion, optimizer, epoch, use_cuda):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        elif args.transform_type == "cutmix":
            targets_a = targets
            targets_b = targets
            lam = 1.0
            if np.random.rand() < 0.5:
                lam = np.random.beta(1.0, 1.0)
                rand_index = torch.randperm(inputs.size(0)).cuda() if use_cuda else torch.randperm(inputs.size(0))
