...
OPEN_CLIP_VIT_B32_PRETRAINED = "laion2b_s34b_b79k"
OPEN_CLIP_VIT_B32_LOCAL_WEIGHT = "save/open_clip_vit_b32_laion2b_s34b_b79k_pretrained.pth"


def _load_open_clip_weight(model, weight_path):
...
def create_model(model_name, num_classes=1000, pretrained=False,
    ...
    if model_name == "resnet50":
        ...
        explicit_clip_weight = os.environ.get("OPEN_CLIP_PRETRAINED", "")
        local_clip_weight = explicit_clip_weight if explicit_clip_weight else OPEN_CLIP_VIT_B32_LOCAL_WEIGHT
        has_local_clip_weight = os.path.isfile(local_clip_weight)
        pretrained_version = OPEN_CLIP_VIT_B32_PRETRAINED if pretrained and not has_local_clip_weight else None
        model, _, preprocess = open_clip.create_model_and_transforms(
            'ViT-B-32',
            pretrained=pretrained_version,
            cache_dir=cache_dir,
        )
        if pretrained and has_local_clip_weight:
            _load_open_clip_weight(model, local_clip_weight)
            print(f"Loaded local OpenCLIP weights from: {local_clip_weight}")
        elif pretrained:
            print(f"Loaded OpenCLIP pretrained weights: ViT-B-32/{OPEN_CLIP_VIT_B32_PRETRAINED}")
        else:
            print("OpenCLIP ViT-B-32 initialized without pretrained weights.")
